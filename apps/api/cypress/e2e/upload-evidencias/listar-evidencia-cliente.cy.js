...
describe('GET /api/evidences - Listagem de evidências do cliente', () => {
  let token
  let clientId
  ...
  it('deve listar as evidências vinculadas ao cliente', () => {
    const cliente = {
    ...
    cy.api({
      ...
      cy.api({
        ...
        cy.api({
          method: 'GET',
          url: `http://localhost:3030/api/evidences?entityType=client&clientId=${clientId}`,
          headers: {
            Authorization: `Bearer ${token}`,
          },
        }).then((response) => {
